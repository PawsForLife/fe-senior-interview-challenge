...
1.  **Client-Side Filtering:** Add buttons to filter the visible products by
    their `category`.
2.  **Loading State:** Implement a loading state (e.g., skeletons) that displays
    while the data is being fetched.
3.  **URL State:** If filtering is implemented, update the URL's query
    parameters to reflect the active filter using the
